...
<template>
  ...
  <div class="greetings">
    ...
    <br>
    <div v-if="store.state.filterdvms.length === 0" style="text-align: center">
  ...
  <div class=" relative space-y-3">
    <div class="grid grid-cols-1 gap-6">
      <div v-for="dvm in dvms" :key="dvm.id"
        ...
        <div className="card-body">
          ...
          <div className="card-actions justify-end mt-auto">
            <div className="tooltip mt-auto" style="border-width: 0">
              ...
              <button
                  v-if="dvm.status !== 'finished' && dvm.status !== 'paid' && dvm.status !== 'payment-required' && dvm.status !== 'subscription-required' && dvm.status !== 'subscription-success' && dvm.status !== 'error' && dvm.status !== 'announced'"
       ...
       store.commit('set_filter_dvms', dvms)"><span
                      class="relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
                Mute All
                  </span></button>
                ...
                Unfollow All
                  ...
                  <div v-for="result in dvm.result">
                    <div v-if="result.active === true">
                      <div
                          v-if="dvm.action === 'mute' && store.state.mutes.find(x => x === result.authorid) === undefined || dvm.action !== 'mute'">
                        <div v-if="result.active === true" className="card w-70 bg-base-200 shadow-xl flex flex-col">
                          ...
                          <div className="playeauthor-wrapper" style="margin-left: 10px; margin-top: 10px">
                            <figure className="w-28">
                              <img v-if="result.event.profile.picture" :src="result.event.profile.picture"
                                   alt="DVM Picture" className="avatar"/>
                              <img v-else class="avatar" src="@/assets/nostr-purple.svg"/>
                            </figure>
                            <div>

                              <a :href="result.authorurl" class="purple" target="_blank">{{
                                  result.event.profile.name
                                }}</a>

                              <a v-if="result.event.profile.nip05 !== undefined && result.event.profile.nip05 !== '' "
                                 :href="result.authorurl" class="purple" target="_blank">({{
                                  result.event.profile.nip05
                                }})</a>
                              <a v-if="store.state.followings.find(x => x == result.authorid) !== undefined" className="white"
